...
public:
  RCLCPP_SMART_PTR_DEFINITIONS(SoundInput)

  /**
   * @brief Constructs a SoundInput object and declares necessary
   * parameters.
   * @param parent Shared pointer to the lifecycle node managing this
   * SoundInput instance.
   */
  explicit SoundInput(rclcpp_lifecycle::LifecycleNode::SharedPtr parent)
      : Afferent("sound_input", parent) {
    RCLCPP_DEBUG(parent_->get_logger(), "Afferent created: [SoundInput]");
  }

  /**
   * @brief Configures the SoundInput by creating subscribers for each
   * specified topic.
   *
...
/// Registers the SoundInput component with the ROS 2 class loader
CS_REGISTER_COMPONENT(SoundInput)
