...
/**
 * @class SoundSupervisionBT
 * @brief Core component execute the audio supervision behaviour.
 */
class SoundSupervisionBT : public cs4home_core::Core {
public:
  RCLCPP_SMART_PTR_DEFINITIONS(SoundSupervisionBT)

  /**
   * @brief Constructs an SoundSupervisionBT object and initializes the parent
   * lifecycle node.
   * @param parent Shared pointer to the lifecycle node managing this
   * SoundSupervisionBT instance.
   */

  explicit SoundSupervisionBT(rclcpp_lifecycle::LifecycleNode::SharedPtr parent)
      : Core("sound_supervision_bt", parent) {
    RCLCPP_DEBUG(parent_->get_logger(), "Core created: [SoundSupervisionBT]");
  }

  /**
   * @brief Configures the SoundSupervisionBT component.
   * @return True if configuration is successful.
   */
  bool configure() override {
    RCLCPP_DEBUG(parent_->get_logger(), "[AudioSupervision] configured");
    ...
    factory_.registerFromPlugin(loader_.getOSName("is_audio_located_bt_node"));

    auto cascade_node =
        std::make_shared<rclcpp_cascade_lifecycle::CascadeLifecycleNode>(
            "audio_supervision_cn");
    ...
    publisher_zmq_ = std::make_shared<BT::PublisherZMQ>(tree_, 10, 1666, 1667);
  ...
  bool activate() override {
    bt_thread_ = std::thread(&SoundSupervisionBT::runBehaviorTree, this);
    return true;
  }
...
/// Registers the SoundSupervisionBT component with the ROS 2 class loader
CS_REGISTER_COMPONENT(SoundSupervisionBT)
