...
public:
  RCLCPP_SMART_PTR_DEFINITIONS(EmergencyBT)

  /**
   * @brief Constructs an EmergencyBT object and initializes the parent
   * lifecycle node.
   * @param parent Shared pointer to the lifecycle node managing this
   * EmergencyBT instance.
   */

  explicit EmergencyBT(rclcpp_lifecycle::LifecycleNode::SharedPtr parent)
      : Core("emergency_bt", parent) {
    RCLCPP_DEBUG(parent_->get_logger(), "Core created: [EmergencyBT]");
  }

  /**
   * @brief Configures the EmergencyBT component.
   * @return True if configuration is successful.
   */
  bool configure() override {
    RCLCPP_DEBUG(parent_->get_logger(), "[EmergencyBT] configured");

    std::string pkgpath =
        ament_index_cpp::get_package_share_directory("bt_test");
    std::string xml_file = pkgpath + "/bt_xml/emergency_action.xml";
  ...
  bool activate() override {
    bt_thread_ = std::thread(&EmergencyBT::runBehaviorTree, this);
    return true;
  }
...
/// Registers the EmergencyBT component with the ROS 2 class loader
CS_REGISTER_COMPONENT(EmergencyBT)
